...
public class BannerCircleIndicator extends View {
	...
	public BannerCircleIndicator(Context context, AttributeSet attrs, int defStyle) {
	...
	public void setBannerGallery(BannerGallery bannerGallery) {
		if (null != mBannerGallery) {
			mBannerGallery.removeOnItemSelectedListener(mPageListener);
		}
		mBannerGallery = bannerGallery;
		if (null != mBannerGallery) {
			mBannerGallery.addOnItemSelectedListener(mPageListener);
		}
	...
	private int getViewPageCount() {
		return (null != mBannerGallery && null != mBannerGallery.getAdapter()) ?
				mBannerGallery.getAdapter().getDataCount() : 0;
	}
	
	private int getViewPageCurrentItem() {
		return hasItems() ? mBannerGallery.getSelectedItemPosition() : -1;
	}
	...
	private class PageListener implements OnItemSelectedListener {
